...
function image_maker($text1,$text,$date,$protocol,$i){
    ...
    header( "Content-type: image/png" );
   // imagepng($image);
   if (!file_exists("sandwich_horizontal_new/info_layer_$i")) {
    mkdir("sandwich_horizontal_new/info_layer_$i", 0777, true);
}
    $save = "sandwich_horizontal_new/info_layer_$i/".strtolower("info_layer_$i") .".png";
    imagepng($image, $save);
    //imagedestroy($image);
}
?>