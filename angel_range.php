...
$db=getDB();
$array=array();
$statement=$db->prepare("SELECT * from info3  LIMIT 169");
$statement->execute();
...
function image_maker($text1,$text,$date,$protocol,$i){
    ...
    imagettftext($image, 16, 90, 1950,470, $text_color,$font,"$ " . strtoupper($text));
    imagettftext($image, 16, 90, 1950,1000, $text_color,$font, "DATE: ".strtoupper($date));
    imagettftext($image, 16, 90, 1950,1950, $text_color,$font, $text1);

    header( "Content-type: image/png" );
   // imagepng($image);
   if (!file_exists("sandwich_angel_0_1_new/info_layer_$i")) {
    mkdir("sandwich_angel_0_1_new/info_layer_$i", 0777, true);
}
    $save = "sandwich_angel_0_1_new/info_layer_$i/".strtolower("info_layer_$i") .".png";
    imagepng($image, $save);
}
?>